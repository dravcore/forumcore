  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      <div className="mb-10 text-center">
        <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Topluluğa Hoş Geldiniz</h1>
        <p className="mt-3 text-muted-foreground">Sorularını sor, deneyimlerini paylaş, topluluğa katkıda bulun.</p>
      </div>
        ...
        {categories.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed py-16 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Admin panelinden ilk kategoriyi ekleyebilirsin.</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {categories.map((category) => (
              <Link key={category.id} href={`/c/${category.slug}`} className="group flex items-center gap-4 rounded-lg border bg-card p-4 transition-colors hover:bg-accent/40">
